        ...
        <div className="flex items-center gap-3">
          ...
          <Link to="/" className="group flex items-center gap-2">
            ...
            <span className="font-extrabold tracking-tight">
              ForgeFlow
              <span className="ml-2 hidden text-xs font-semibold text-muted-foreground md:inline-block">{pathname}</span>
            </span>
          </Link>
        </div>
        <div className="flex items-center gap-2">
          {title ? <span className="text-sm text-muted-foreground hidden sm:block">{title}</span> : null}
        </div>
      </div>
    </header>
  );
